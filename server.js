...
async function accountBlockedReason(userId) {
  ...
  try {
    ...
    if (snap.exists) {
      ...
      /* ============================================================
         حسابٌ لا وجود له = حسابٌ محظور. وليس العكس.

         كانت `snap.exists` الكاذبة تُسقط إلى `reason = null` — أي
         «مرحباً، تفضّل». فمن حذفتَه نهائياً من اللوحة يبقى **توكنه**
         في جيبه صالحاً تسعين يوماً، وكل فحص حظر يفتح له الباب لأن
         لا مستند يقول غير ذلك.

         والأثر ليس نظرياً: مندوبٌ محذوف وتطبيقُه مفتوح كان يواصل
         رؤية الطلبات وقبولها **وتحصيل كاش الزبائن** — وأنت تحسبه
         خارج النظام منذ أسبوع.

         القاعدة الوحيدة الصحيحة: الهوية تُثبت بوجود سجلّها. توكنٌ
         صالح لحسابٍ غير موجود ليس «حالة غريبة نتسامح معها»، هو
         التعريف الحرفي لحسابٍ أُنهي.

         (عطل الشبكة ليس هذه الحالة: فشلُ القراءة يذهب إلى catch
         أدناه ويُرجع null عمداً — لا نطرد مستخدماً سليماً لأن
         Firestore تعثّرت لحظة.)
         ============================================================ */
      reason = 'حسابك لم يعد موجوداً — سجّل من جديد أو تواصل مع الإدارة';
    }
  } catch (e) {
...
app.post('/api/auth/register', async (req, res) => {
  try {
    ...
    if (db) {
      ...
      /* ============================================================
         الرقم لا يتكرّر — كما لا يتكرّر البريد.

         كان الحارس على البريد وحده. والرقم يُترك، فيصير للشخص الواحد
         حساباتٌ بعدد المرّات التي نسي فيها بريده. وهذا ما وقع فعلاً:
         ثلاثة حسابات برقم 0593654276، وحسابا مندوب غيرها.

         وثمنه لم يكن إرباكاً في قائمة، بل يوماً كاملاً من التشخيص:

           · إشعار الزبون يذهب لحساب ولا جهاز عليه
           · إشعار المندوب يبحث عن حساب ويجد آخر
           · الطلب «ينتقل» بين الحسابات لأن الرقم كان دليل الملكية
           · وصاحب المنصّة يبدّل بين حساباته وهو لا يدري

         والرقم أصدق من البريد هنا: من ينسى بريده لا ينسى رقمه، وهو
         الذي يُطبع على الطلب ويتصل به المندوب. فحراستُه أولى.

         والمقارنة مُطبَّعة (`+970` و`05` رقمٌ واحد) — وإلا مرّ التكرار
         من باب اختلاف الصيغة، وهو نفس العطل الذي أصلحناه في ثلاثة
         مواضع أخرى اليوم.
         ============================================================ */
      const normNew = normPhone(phone) || String(phone || '').trim();
      if (normNew) {
        const dupPhone = await db.collection('users')
          .where('phone', '==', normNew)
          .limit(1)
          .get();
        meter.addReads(1, 'فحص تكرار الرقم');

        if (!dupPhone.empty) {
          const who = dupPhone.docs[0].data() || {};
          console.warn(`⛔ تسجيل برقم مكرَّر: ${normNew} — موجود لدى ${who.name || dupPhone.docs[0].id}`);
          return res.status(400).json({
            success: false,
            error: 'هذا الرقم مسجَّل عندنا — سجّل دخولاً بدل حسابٍ جديد. ولو نسيت كلمة السرّ تواصل مع إدارة زادنا 📞',
            phoneTaken: true
          });
        }
      }
      ...
      if (userType === 'restaurant' && ownedRestaurantId && String(ownedRestaurantId).trim()) {
        try {
          ...
          await db.collection('restaurants').doc(createdRestId).set({
            ...
            categories: [],
            menu: [],
            /* النسبة تُكتب رقماً بحسب نوع الشريك، لا نصّاً ثابتاً.
             *
             * كان كل شريك يُولد بـ`'10%'` — بما فيهم السوبرماركت الذي
             * عمولته ٦٪. فيقرأ صاحبه في لوحته رقماً لا يُطبَّق عليه،
             * ويكتشف الفرق عند أول كشف حساب. ونصٌّ فيه `%` لا يُحسب به
             * شيء أصلاً — الحساب من `utils/money`.
             *
             * ونكتبه رقماً (`0.06`) كي يُقرأ ويُقارن ويُعرض بالنسبة التي
             * تريدها الواجهة، ويُستعمل كـ`commissionRate` على الطلب لمن
             * اتفقتَ معه على غير الافتراضي. */
            commissionRate: isMarket
              ? Number(process.env.MARKET_COMMISSION || 0.06)
              : Number(process.env.RESTAURANT_COMMISSION || 0.10),
            status: 'pending',
            isActive: false,
            isOpen: false,
      ...
      if (isPartner) {
        /* ============================================================
           إبطال كاش الشركاء — وهذا هو السبب الحقيقي لـ«وصلني متأخّراً».

           `partners:all` عمرُه خمس دقائق، ولم يكن التسجيل يمسحه. أي أنّ
           الشريك يُكتب في قاعدة البيانات، ثم تسأل اللوحة عن قائمة
           الشركاء فتُجيبها نسخةٌ محفوظة **قبل** أن يسجّل — فلا يظهر
           في جدول الاعتماد حتى تنتهي الدقائق الخمس.

           فكان الخبر يمرّ بطريقين وكلاهما يخذل: السوكت يصل فوراً لكن
           يضيع إن لم تكن اللوحة متصلة تلك اللحظة (ولا يُعاد أبداً)،
           والجدول يصل دائماً لكن متأخّراً خمس دقائق. فما رأيتَه هو
           الطريق الثاني — لأنّ الأول ضاع.

           سطرٌ واحد يجعل القائمة صادقة لحظة كتابتها، فيراه أي استطلاع
           بعدها فوراً. والثمن: قراءةٌ واحدة إضافية عند كل تسجيل شريك —
           وهي أرخص شيء في المنصّة كلّها.
           ============================================================ */
        try { require('./utils/cache').invalidate('partners:all'); } catch (e) {}
        ...
        if (ioRT) {
          const payload = {
            ...
            type: partnerKind,
            // اللوحة تُميّز السوبرماركت من المطعم بهذا الحقل — وبدونه
            // كانت لافتة التنبيه تقول «مندوب» لصاحب ماركت
            partnerType: partnerKind === 'market' ? 'market' : '',
            status: 'pending',
            date: new Date(),
          };
          ioRT.emit('new_partner_request', payload);
          ioRT.to('manager_monitor').emit('new_partner_request', payload);
        }

        /* ============================================================
           إشعار الجوّال — مُطفَأ بطلب صاحب المنصّة.

           كان يُرسَل هنا `notifyManagers` فيصل إشعارٌ إلى هاتفك مع كل
           تسجيل. وطلبتَ ألّا يصلك على الجوّال وأن يبقى على الكمبيوتر
           وحده، فأُطفئ — ولم يُحذف.

           والذي كان يُخيفني من إطفائه أنّ الكمبيوتر قد يكون مغلقاً،
           فيضيع الطلب. وهذا الخوف زال: اللوحة صارت تستطلع الطلبات
           المعلّقة كل دقيقة وتُنبّه على كل ما لم ترَه من قبل — أي أنّ
           الطلب الذي وصل والكمبيوتر مطفأ يُنبّهك عند أول فتح للّوحة،
           بصوت وإشعار نظام. لا يضيع، إنما ينتظرك.

           لإعادته يوماً (سفر، أو إجازة أسبوع): احذف علامتي التعليق عن
           السطور أدناه وارفع الملفّ — لا شيء آخر مطلوب.
           ============================================================ */
        // const kindAr = partnerKind === 'market' ? 'سوبرماركت'
        //              : partnerKind === 'driver' ? 'مندوب' : 'مطعم';
        // const { notifyManagers } = require('./routes/push');
        // notifyManagers(req.app, {
        //   title: `طلب انضمام جديد — ${kindAr} 🤝`,
        //   body: `${name} · ${phone} — بانتظار اعتمادك من اللوحة`,
        //   data: { type: 'partner_request', partnerId: String(userId) },
        // }).catch(() => {});
      }
 ...
 */
/* ============================================================
   GET /api/config/rates — النِّسَب كما هي الآن، من مصدرها الوحيد.

   كانت «١٠٪» محفورةً في أربعة مواضع في اللوحة وفي شاشة اتفاقية
   الشراكة بالتطبيق. فيوم صارت عمولة السوبرماركت ٦٪، كذبت خمسُ شاشات
   دفعةً واحدة — على الشريك الذي يقرأ الاتفاقية قبل أن يوقّع.

   والمسار مفتوح بلا حارس عمداً: النسبة ليست سرّاً، بل هي أول ما
   يسأل عنه أي شريك. وإخفاؤها خلف مفتاحٍ يجعل الواجهات تخترعها.

   ولا يُستعمل في **الحساب** إطلاقاً — الحساب مجمَّدٌ داخل كل طلب.
   هذا للعرض وحده: «كم تأخذون؟».
   ============================================================ */
app.get('/api/config/rates', (req, res) => {
  const m = require('./utils/money');
  res.json({
    restaurant: m.RESTAURANT_COMMISSION,
    market:     m.MARKET_COMMISSION,
    driver:     m.DRIVER_COMMISSION,
    note: 'للعرض فقط — كل طلبٍ يُحاسَب بالنسبة المجمَّدة فيه يوم إنشائه',
  });
});
