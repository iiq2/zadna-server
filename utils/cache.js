...
/**
 * نظرةٌ بلا كتابة: يُرجع النسخة المخزّنة إن كانت طازجة، وإلا `null`.
 *
 * ===== لماذا وُجدت هذه الدالة =====
 *
 * كان في السيرفر أربعة مواضع تقول «اقرأ الكاش ولا تستعلم»، فتكتب:
 *
 *     await cached('orders:all', TTL, async () => []);
 *
 * وهي لا تفعل ما تقول. `cached` ليست قراءةً فقط: إن كان المفتاح بارداً
 * أو منتهياً **نفّذت الدالة وخزّنت نتيجتها**. والنتيجة هنا `[]`.
 *
 * فيُكتب في `orders:all` مصفوفةٌ فارغة بعمر ثلاثين دقيقة كاملة — وكل
 * تطبيق يستطلع بعدها يُقال له: **لا طلبات**. تختفي الطلبات من المندوب
 * ومن المطعم ومن الزبون معاً، لا لأنها أُلغيت بل لأن السيرفر نسيها.
 *
 * وأخبث المواضع الأربعة حارسُ التكرار: يعمل عند **كل طلب جديد**. فمن
 * يطلب والكاش بارد يمحو كل الطلبات القائمة، ثم يُلصق طلبه وحده في
 * القائمة الفارغة. طلبٌ يظهر، وما قبله يختفي.
 *
 * القاعدة المستفادة: دالةٌ اسمها «اقرأ» لا يجوز أن تكتب. وإن كتبت،
 * فلا تُعطَ دالةَ تحميلٍ كاذبة لتُسكِتها — تُعطى دالةً تقرأ فعلاً، أو
 * تُستبدل بدالة نظر لا تكتب. وهذه هي.
 */
...
module.exports = { cached, peekCached, invalidate, updateCached };
