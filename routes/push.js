...
router.post('/fcm_token', needsIdentity, async (req, res) => {
  try {
    ...
    /* ===== علم «يعمل مندوباً» — خارج شرط تغيّر الجهاز =====
     *
     * وضعتُه أولاً **داخل** كتلة `if (!known || تغيّر الوسم)`. وكان
     * ميتاً لمن يحتاجه بالضبط: جهاز مسجَّل سلفاً بوسم `captain` لا
     * يُحقّق الشرط، فلا تعمل الكتلة، فلا يُرفع العلم أبداً.
     *
     * ولا الخروجُ والدخول كان ينقذه — رمز FCM لا يتغيّر بتبديل
     * الجلسة، فيبقى `known` موجوداً ووسمه هو هو.
     *
     * القاعدة: شرطُ «هل تغيّر الجهاز؟» يخصّ **مصفوفة الأجهزة** وحدها.
     * وحقيقةُ «هذا الشخص يعمل مندوباً» لا علاقة لها بتغيّر الرمز —
     * تُكتب كلّما فتح تطبيق الكابتن، مرّةً كانت أو ألفاً.
     *
     * `merge` لا يمسّ شيئاً آخر، والكتابة رخيصة (مرّة عند كل إقلاع). */
    if (app === 'captain' && d.worksAsDriver !== true) {
      await ref.update({ worksAsDriver: true });
      console.log(`🛵 وُسم مندوباً: ${d.name || '—'} [${uid}]`);
    }
