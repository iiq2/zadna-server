...
router.patch('/:id', needsIdentity, async (req, res) => {
    ...
    await docRef.update(updateData);
      ...
      /* ============================================================
         ...
         فعلاً أهمّ من عدّاد. */
      /* `cur.status !== 'DELIVERED'` — حارسُ التكرار الذي كان ينقص.
       *
       * `cur` هي نسخة الطلب **قبل** التحديث. فبلا هذا الشرط، أيّ PATCH
       * ثانٍ بحالة DELIVERED على طلبٍ سُلّم أصلاً يزيد كاش المندوب
       * مرّةً أخرى. وليس افتراضاً نظرياً: شبكة نابلس تُسقط الردّ بعد أن
       * ينجح الحفظ، فيُعيد التطبيق المحاولة — وهو بعينه السيناريو الذي
       * بُني له حارسُ الطلب المكرَّر أعلى الملف، وغاب هنا وحده.
       *
       * والكاش المضخَّم ضررُه مزدوج: يرفع المندوب فوق سقف الكاش (٨٠٠ ₪)
       * فتتوقّف عنه الطلبات وهو شغّال، ويُظهره في شاشة الحذف مديناً بأكثر
       * ممّا عليه. `increment` ذرّي لكنه لا يَعرف أنه كُرِّر — فالشرط هنا. */
      if (status === 'DELIVERED' && String(cur.status || '') !== 'DELIVERED') {
        try {
          const m = cur.money || {};
