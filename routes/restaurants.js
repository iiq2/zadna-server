...
router.get('/', async (req, res) => {
    try {
      ...
      /* ============================================================
         ...
         ============================================================ */
      /* السوبرماركت لا يظهر بين المطاعم.
       *
       * الاثنان في نفس المجموعة عمداً (يرث الماركت الطلبات والمحفظة
       * والشات والإشعارات بلا سطر إضافي)، لكن الزبون يراهما في مكانين
       * مختلفين: المطاعم في تبويبها، والماركت في تبويبه — وواحدٌ فقط،
       * أقربُهم إليه.
       *
       * وبلا هذا السطر يظهر «سوبرماركت الفلاني» بين المطاعم بمنيو
       * فارغ (بضاعته في مجموعة فرعية لا في `menu`)، فيدخله الزبون
       * ويجده خالياً ويظنّ المنصّة خربانة.
       *
       * والإدارة ترى الجميع — هي تحتاج الصورة كاملة. */
      const asksAll = req.query.all === '1' || req.query.all === 'true';
      const showAll = asksAll && !!req.isAdmin;
      /* نُنشئ قائمةً جديدة ولا نعدّل المُكاشة.
       * `restaurants` تأتي من الكاش، وأي `filter` بإسنادٍ فوقها كان
       * سيُفسدها لكل طلبٍ لاحق خلال العشر دقائق — بما فيه طلب الإدارة. */
      const forCustomer = req.isAdmin
        ? restaurants
        : restaurants.filter(r => String(r.partnerType || 'restaurant') !== 'market');
      if (asksAll && !req.isAdmin) {
        console.warn('⚠️ طلب ?all=1 بلا مفتاح إدارة — عُرضت المطاعم المعتمدة فقط');
      }
      const visible = showAll
        ? forCustomer
        : forCustomer.filter(r => (!r.status || r.status === 'approved') && r.isOpen !== false);
