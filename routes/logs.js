...
/* ============================================================
   حدّ كتابة السجلّ — الباب الوحيد المفتوح للإنترنت بلا هوية.

   وهو مفتوح **عمداً وبحقّ**: عطلٌ يقع قبل تسجيل الدخول لا هوية معه،
   واشتراط الهوية يعمي أعيننا عن أخطاء شاشة الدخول نفسها — وهي أكثر
   ما يشتكي منه الناس.

   لكن المفتوح بلا حدّ يُستغلّ: نداءٌ في حلقة يكتب عشرين ألف سجلّ في
   دقائق، فتنفد حصة **الكتابة** اليومية — ومعها يتوقّف كل شيء: لا طلب
   يُحفظ، ولا حالة تتغيّر، ولا رسالة تُكتب. أي أن أرخص هجوم ممكن
   يُسقط المنصّة كلها.

   فالحلّ ليس الإغلاق بل الحدّ: عشرون سجلاً في الدقيقة لكل عنوان.
   يتّسع لأسوأ عطل حقيقي (تطبيق يرمي أخطاءً متتابعة)، ويخنق العابث
   من أول دقيقة. والذاكرة تُنظَّف من العناوين الخاملة فلا تكبر بلا حدّ.

   والردّ 429 لا 403: نقول «تجاوزت الحدّ» لا «ممنوع» — الفرق يعني
   للمطوّر الحقيقي شيئاً وللمهاجم لا شيء.
   ============================================================ */
const LOG_WINDOW_MS = 60000;
const LOG_MAX_PER_IP = 20;
const _logHits = new Map();   // عنوان → [طوابع زمنية]

function logRateOk(ip) {
  const now = Date.now();
  const arr = (_logHits.get(ip) || []).filter(t => now - t < LOG_WINDOW_MS);
  arr.push(now);
  _logHits.set(ip, arr);
  // تنظيف دوري: لا نُبقي عنواناً لم يكتب منذ دقيقتين
  if (_logHits.size > 500) {
    for (const [k, v] of _logHits) {
      if (!v.length || now - v[v.length - 1] > 2 * LOG_WINDOW_MS) _logHits.delete(k);
    }
  }
  return arr.length <= LOG_MAX_PER_IP;
}
...
router.post('/logs', async (req, res) => {
  try {
    ...
    if (!logRateOk(ip)) {
      return res.status(429).json({ success: false, error: 'تجاوزت حدّ تسجيل الأخطاء — أعد المحاولة بعد دقيقة' });
    }
    const b = req.body || {};
    const entry = {
